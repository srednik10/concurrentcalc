 ...
 */
public class FullHouse4s implements PokerHand{
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        if (map.entrySet().stream().filter(e -> e.getValue() != 0).count() == 2 && map.entrySet().stream().anyMatch(e -> e.getValue() != 0 && e.getKey() == '4')) {
            Character valueOfPair = map.entrySet().stream().filter(e -> e.getValue() != 0 && e.getKey() != '4').findFirst().get().getKey();
            ...
            if (countInDeadCards < 2 && countValueOfPairInDeadCards < 3 && countValueOfPairInCards < 3) {
                for(int i=0;i<3-countInCards;i++) producedDeadCards.add(new PokerCard("4x"));
            ...
            if((map.entrySet().stream().filter(e -> e.getValue() != 0).count() == 1 && (cards.size() < 3 || cards.size() == 3 && map.entrySet().stream().findFirst().filter(e -> e.getValue() != 0).get().getKey() == '4') ) || cards.size() == 0 )  return true;
            else return false;
        }
    }
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        long combos;
        int countInCards = (int)cards.stream().filter(e -> e.value == '4').count();
        int cardsNeeded = 3 - (int)cards.stream().filter(e -> e.value == '4').count();
        int cardsAvaiable = 4 - (int)deadCards.stream().filter(e -> e.value == '4').count();
        if(cardsAvaiable >= cardsNeeded) combos = CombinatoricsUtils.binomialCoefficient(cardsAvaiable,cardsNeeded);
        else return  0;
        for(int i=0 ; i<cardsNeeded;i++) comboTemporaryDeadCards.add(new PokerCard(new StringBuilder().append('4').append('x').toString()));
        if(cards.size()-countInCards == 1){
            Character valueOfPair = cards.stream().filter(e -> e.value != '4').findFirst().get().value;
