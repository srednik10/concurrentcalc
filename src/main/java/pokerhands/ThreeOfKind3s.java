 ...
 */
public class ThreeOfKind3s implements PokerHand {
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        cards.forEach(e -> {
            if (e.value != '3') listOfDifferent.add(e);
        });
        ...
        else if(countInCards == 2 && countInDeadCards < 2 && cards.size() < 5) {for(int i=0;i<1;i++) producedDeadCards.add(new PokerCard("3x")); return true;}
        else if(countInCards == 1 && countInDeadCards < 2 && cards.size() < 4) {for(int i=0;i<2;i++) producedDeadCards.add(new PokerCard("3x")); return true;}
        else if(countInCards == 0 && countInDeadCards < 2 && cards.size() < 3) {for(int i=0;i<3;i++) producedDeadCards.add(new PokerCard("3x")); return true;}
        else return false;
    }
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        int countDifferent = (int) cards.stream().filter(e -> e.value != '3').count();
        int cardsNeeded = 3 - (int)cards.stream().filter(e -> e.value == '3').count();
        int cardsAvailable = 4 - (int) deadCards.stream().filter(e -> e.value == '3').count();
        long combos;
        if(cardsAvailable >= cardsNeeded) combos = CombinatoricsUtils.binomialCoefficient(cardsAvailable,cardsNeeded);
        else return  0;
        for (int i=0 ; i < cardsNeeded ; i++) comboTemporaryDeadCards.add(new PokerCard("3x"));
