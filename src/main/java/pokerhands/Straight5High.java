 ...
 */
public class Straight5High implements PokerHand {
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        if(cards.stream().allMatch(e -> e.value == 'A' || e.value == '2' || e.value == '3' || e.value == '4' || e.value == '5')){
            ...
            if(map.entrySet().stream().allMatch(entry -> (entry.getValue() ==  0  && deadCards.stream().filter(e -> e.value == entry.getKey()).count() < 4) || entry.getValue() == 1)){
                if(cards.stream().noneMatch(e -> e.value == 'A')) producedDeadCards.add(new PokerCard(new StringBuilder().append('A').append('x').toString()));
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        long combos = 1;
        if(cards.stream().filter(e -> e.value == 'A').count() == 0) {combos = combos * (4 - deadCards.stream().filter(e -> e.value == 'A').count()); comboTemporaryDeadCards.add(new PokerCard("Ax"));}
