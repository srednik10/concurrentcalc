 ...
 */
public class ThreeOfKindJs implements PokerHand {
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        cards.forEach(e -> {
            if (e.value != 'J') listOfDifferent.add(e);
        });
        ...
        else if(countInCards == 2 && countInDeadCards < 2 && cards.size() < 5) {for(int i=0;i<1;i++) producedDeadCards.add(new PokerCard("Jx")); return true;}
        else if(countInCards == 1 && countInDeadCards < 2 && cards.size() < 4) {for(int i=0;i<2;i++) producedDeadCards.add(new PokerCard("Jx")); return true;}
        else if(countInCards == 0 && countInDeadCards < 2 && cards.size() < 3) {for(int i=0;i<3;i++) producedDeadCards.add(new PokerCard("Jx")); return true;}
        else return false;
    }
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        for (int i=0 ; i < cardsNeeded ; i++) comboTemporaryDeadCards.add(new PokerCard("Jx"));
