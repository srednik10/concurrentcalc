 ...
 */
public class FourOfKindKs implements PokerHand {
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        else if(countInCards == 3 && countInDeadCards == 0 && cards.size() < 5) {producedDeadCards.add(new PokerCard("Kx")); return true;}
        else if(countInCards == 2 && countInDeadCards == 0 && cards.size() < 4) {for(int i=0;i<2;i++) producedDeadCards.add(new PokerCard("Kx"));return true;}
        else if(countInCards == 1 && countInDeadCards == 0 && cards.size() < 3) {for(int i=0;i<3;i++) producedDeadCards.add(new PokerCard("Kx"));return true;}
        else if(countInCards == 0 && countInDeadCards == 0 && cards.size() < 2) {for(int i=0;i<4;i++) producedDeadCards.add(new PokerCard("Kx"));return true;}
        else return false;
    }
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        int cardsNeeded = 4 - (int)cards.stream().filter(e -> e.value == 'K').count();
