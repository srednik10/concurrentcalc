 ...
 */
public class FullHouse3s implements PokerHand {
    ...
    public boolean canMake(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        long countInDeadCards = deadCards.stream().filter(e -> e.value == '3').count();
        ...
        if (map.entrySet().stream().filter(e -> e.getValue() != 0).count() == 2 && map.entrySet().stream().anyMatch(e -> e.getValue() != 0 && e.getKey() == '3')) {
            Character valueOfPair = map.entrySet().stream().filter(e -> e.getValue() != 0 && e.getKey() != '3').findFirst().get().getKey();

            long countInCards = cards.stream().filter(e -> e.value == '3').count();
            ...
            if (countInDeadCards < 2 && countValueOfPairInDeadCards < 3 && countValueOfPairInCards < 3) {
                for(int i=0;i<3-countInCards;i++) producedDeadCards.add(new PokerCard("3x"));
            ...
            if((map.entrySet().stream().filter(e -> e.getValue() != 0).count() == 1 && (cards.size() < 3 || cards.size() == 3 && map.entrySet().stream().findFirst().filter(e -> e.getValue() != 0).get().getKey() == '3') ) || cards.size() == 0 )  return true;
            else return false;
        }
    }
    ...
    public long getCountOfHandCombos(List<PokerCard> cards, List<PokerCard> deadCards) {
        ...
        long combos;
        int countInCards = (int)cards.stream().filter(e -> e.value == '3').count();
        int cardsNeeded = 3 - (int)cards.stream().filter(e -> e.value == '3').count();
        int cardsAvaiable = 4 - (int)deadCards.stream().filter(e -> e.value == '3').count();
        if(cardsAvaiable >= cardsNeeded) combos = CombinatoricsUtils.binomialCoefficient(cardsAvaiable,cardsNeeded);
        else return  0;
        for(int i=0 ; i<cardsNeeded;i++) comboTemporaryDeadCards.add(new PokerCard(new StringBuilder().append('3').append('x').toString()));
        if(cards.size()-countInCards == 1){
            Character valueOfPair = cards.stream().filter(e -> e.value != '3').findFirst().get().value;
